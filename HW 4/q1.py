...
def accuracy_score(y_pred, y_true):
    ...
    correct = np.sum((y_pred - y_true) == 0)
...
def train_test_split(X, y, test_size):
    ...
    idx = np.linspace(0, N - 1, N, dtype=np.int32)
...
def k_fold_cv(X, y, k_folds=5, shuffle=True):
    ...
    for k in range(0, k_folds):
        partition = k * div
        train_idx = np.concatenate([idx[0:partition], idx[partition + div:]])
        test_idx = idx[partition: partition + div]
        X_train = X[train_idx, :]
        y_train = y[train_idx]
        X_test = X[test_idx, :]
        y_test = y[test_idx]
